...
from chemprop.features.features_generators import get_available_features_generators
...
def add_train_args(parser: ArgumentParser):
    ...
    parser.add_argument('--use_compound_names', action='store_true', default=False,
                        help='Use when test data file contains compound names in addition to SMILES strings')#当测试数据文件除 SMILES 字符串外还包含复合名称时使用
    parser.add_argument('--max_data_size', type=int,
                        help='Maximum number of data points to load')#最大数据加载数
    parser.add_argument('--test', action='store_true', default=False,
                        help='Whether to skip training and only test the model')#是否只测试

    parser.add_argument('--features_only', action='store_true', default=False,
                        help='Use only the additional features in an FFN, no graph network')#使用图特征之外的特征
    parser.add_argument('--features_generator', type=str, nargs='*',
                        choices=get_available_features_generators(),
                        help='Method of generating additional features')#指定特征生成器
    parser.add_argument('--features_path', type=str, nargs='*',
                        help='Path to features to use in FNN (instead of features_generator)')#是否加载特征文件

    parser.add_argument('--save_dir', type=str, default='./ckpt',
                        help='Directory where model checkpoints will be saved')#保存模型检查点的目录
    parser.add_argument('--save_smiles_splits', action='store_true', default=False,
                        help='Save smiles for each train/val/test splits for prediction convenience later')#保存训练/验证/测试划分
    parser.add_argument('--checkpoint_dir', type=str, default=None,
                        help='Directory from which to load model checkpoints'
                             '(walks directory and ensembles all models that are found)')#加载模型检查点的目录
    parser.add_argument('--checkpoint_path', type=str, default=None,
                        help='Path to model checkpoint (.pt file)')#加载预训练模型
    parser.add_argument('--dataset_type', type=str,
                        choices=['classification', 'regression', 'multiclass'],
                        help='Type of dataset, e.g. classification or regression.'
                             'This determines the loss function used during training.',
                        default='regression') # classification，数据集任务类型
    parser.add_argument('--multiclass_num_classes', type=int, default=3,
                        help='Number of classes when running multiclass classification')#多分类数量
    parser.add_argument('--separate_train_path', type=str,default=None,
                        help='Path to separate train set, optional')#独立训练集路径
    parser.add_argument('--separate_val_path', type=str,default=None,
                        help='Path to separate val set, optional')#独立验证集路径
    parser.add_argument('--separate_val_features_path', type=str, nargs='*',
                        help='Path to file with features for separate val set')#独立验证集特征
    parser.add_argument('--separate_test_path', type=str,default=None,
                        help='Path to separate test set, optional')#独立测试集路径
    parser.add_argument('--separate_test_features_path', type=str, nargs='*',
                        help='Path to file with features for separate test set')#独立测试集特征
    parser.add_argument('--split_type', type=str, default='scaffold_balanced',
                        ...
                        help='Method of splitting the data into train/val/test')#数据集划分方法
    parser.add_argument('--split_sizes', type=float, nargs=3, default=[0.8, 0.1, 0.1],
                        help='Split proportions for train/validation/test sets')#数据集划分比例
    parser.add_argument('--num_runs', type=int, default=1,
                        help='Number of runs when performing k independent runs')#独立运行次数
    parser.add_argument('--folds_file', type=str, default=None,
                        help='Optional file of fold labels')#不同划分标签
    ...
    parser.add_argument('--crossval_index_dir', type=str,
                        help='Directory in which to find cross validation index files')#交叉验证索引文件所在目录
    parser.add_argument('--crossval_index_file', type=str,
                        help='Indices of files to use as train/val/test'
                             'Overrides --num_folds and --seed.')#用作 train/val/test 的文件索引
    ...
    parser.add_argument('--quiet', action='store_true', default=False,
                        help='Skip non-essential print statements')#跳过非必要的打印语句
    parser.add_argument('--log_frequency', type=int, default=10,
                        help='The number of batches between each logging of the training loss')#每次记录训练损耗之间的批次数
    parser.add_argument('--no_cuda', action='store_true', default=False,
                        help='Turn off cuda')
    parser.add_argument('--show_individual_scores', action='store_true', default=False,
                        help='Show all scores for individual targets, not just average, at the end')
    parser.add_argument('--no_cache', action='store_true', default=False,
                        help='Turn off caching mol2graph computation')#缓存功能
    ...
    parser.add_argument('--no_features_scaling', action='store_true', default=False,
                        help='Turn off scaling of features')#特征缩放
    ...
    parser.add_argument('--ensemble_size', type=int, default=1,
                        help='Number of models in ensemble')#组合模型数量
    ...
    parser.add_argument("--dump_path", default="dumped", type=str,
                        help="Experiment dump path")#实验转存路径
    ...
    parser.add_argument("--runs", type=str, default="default_run", help="Experiment run name")
