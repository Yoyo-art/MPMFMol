...
from chemprop.data.data import MoleculeDataset, pre_MoleculeDataset
from chemprop.data.scaler import StandardScaler
from chemprop.data.untils import get_task_names, get_data, load_data, split_data, get_class_sizes, get_pre_data
from chemprop.models.model import build_model, build_pretrain_model
from chemprop.nn_utils import param_count
from chemprop.torchlight.utils import snapshot
from chemprop.train.evaluate import evaluate, evaluate_predictions
from chemprop.train.predict import predict
from chemprop.train.train import train
from chemprop.utils import get_loss_func, get_metric_func, makedirs, save_checkpoint, build_optimizer, \
    build_lr_scheduler, Early_stop, load_checkpoint
...
def run_training(args: Namespace, pretrain: bool, logger: Logger = None) -> List[float]:
    ...
    for model_idx in range(args.ensemble_size):
        ...
        #     add_FUNC_prompt(model, args)  # 提示生成器
        debug(model)
        ...
        for epoch in range(args.epochs):
            ...
            if args.minimize_score and avg_test_score < best_score or \
                    not args.minimize_score and avg_test_score > best_score:
                best_score, best_epoch = avg_test_score, epoch
                save_checkpoint(os.path.join(save_dir, 'model.pt'), model, scaler, features_scaler, args)
...
def pre_training(args: Namespace, logger: Logger = None) -> List[float]:
    ...
    for model_idx in range(args.ensemble_size):
        ...
        datas = list(zip(smiles, y1, y2, y3))
        train_smiles, test_smiles = torch.utils.data.random_split(datas, [train_size, test_size])
        ...
        for epoch in range(args.start_epochs, args.end_epochs):
            ...
            for test_batch in test_loader:
                smiles_batch, y1_batch, y2_batch, y3_batch = test_batch
                with torch.no_grad():
            ...
            logger.info(
                f'[{epoch}/{args.epochs}] train loss {total_loss / len(train_loader):.4f},test loss {total_test_loss / len(test_loader):.4f}')