...
class MoleculeDatapoint:
    ...
    def __init__(self,
        ...
        self.smi_enc = label_smiles(self.smiles, CHARISOSMISET, len(self.smiles))

        # Generate additional features if given a generator
        if self.features_generator is not None:
            self.features = []
            features_generator = get_features_generator(self.features_generator)
            if self.mol is not None and self.mol.GetNumHeavyAtoms() > 0:
                self.features.extend(features_generator(self.mol).tolist())
...
class MoleculeDataset(Dataset):
    ...
    def smiles_enc(self) -> List[NDArray[np.int64]]:
        """
        Returns the smiles strings associated with the molecules.

        :return: A list of smiles strings.
        """
        return [d.smi_enc for d in self.data]
...
class pre_MoleculeDataset(Dataset):
    """A MoleculeDataset contains a list of molecules and their associated features and targets."""

    def __init__(self, data: List[pre_MoleculeDatapoint]):
        """
        Initializes a MoleculeDataset, which contains a list of MoleculeDatapoints (i.e. a list of molecules).
    ...
    def y1(self):
        ...
        :return: A list of smiles strings.
        """
        return [d.y1 for d in self.data]
    ...
    def __getitem__(self, item) -> Union[pre_MoleculeDatapoint, List[pre_MoleculeDatapoint]]:
        """
        Gets one or more MoleculeDatapoints via an index or slice.
