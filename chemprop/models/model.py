...
class MoleculeModel(nn.Module):
    ...
    def create_smi_encoder(self, args: Namespace):
        ...
        """
        from .cmpn import modelTransformer_smi
    ...
    def forward(self, *input):
        ...
        if not self.pretrain:
            ...
            # ablate_feature = 'h_s'
            # if ablate_feature == 'h_s':
            #     # 打乱序列特征
            #     shuffled_idx = torch.randperm(smi_out.size(0), device=smi_out.device)
            #     smi_out = smi_out[shuffled_idx]
            pair = torch.cat([g_emb, smi_out], dim=-1)
            ...
            x = self.relu(x)
            x = self.dropout(x)
            x1 = self.class_classifier1(x)
            x2 = self.class_classifier2(x)
            x3 = self.class_classifier3(x)
...
class Prompt_generator(nn.Module):
    def __init__(self, args:Namespace):
        ...
        self.alpha.data.fill_(0.1)#初始化为0.1
...
class PromptGeneratorOutput(nn.Module):
    ...
    def forward(self,hidden_states: torch.Tensor):
        ...
        # prompt, att = self.prompt_generator(hidden_states)  
